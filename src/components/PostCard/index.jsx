...
const PostCard = ({ post }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-md shadow-md h-full transform transition-transform hover:scale-105">
      <Link to={`/blog${post.meta.canonical}`}>
        <img src={post.pageData.photo.cover} alt={post.pageData.title} className="w-full h-full object-cover" />
      </Link>
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
        ...
        <Link to={`/blog${post.meta.canonical}`} className="block mt-2 text-xl font-bold text-white">
          {post.pageData.title}
        </Link>
